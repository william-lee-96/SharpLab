...
mkdir UKB_REGENIE_step2_5QP_out
mkdir UKB_REGENIE_step2_5QP_log

for pheno in mutation_size expansion_size contraction_size expansion_propensity contraction_propensity
do
	
	mkdir UKB_REGENIE_step2_5QP_out/$pheno
	mkdir UKB_REGENIE_step2_5QP_log/$pheno

	for cohort in SC deCODE
	do

		mkdir UKB_REGENIE_step2_5QP_out/$pheno/$cohort
		mkdir UKB_REGENIE_step2_5QP_log/$pheno/$cohort
		...
		do
			...
			bsub -P acc_sharpa01a -q express -n 16 -R span[hosts=1] -W 8:00 -o UKB_REGENIE_step2_5QP_log/$pheno/$cohort/"$file_bn.log" \
			regenie     \
			    --step 2 \
			    ...
			    --apply-rint \
			    --pred UKB_REGENIE_step1_5QP_out/$pheno/$cohort/"${file_bn}_pred.list" \
			    --out UKB_REGENIE_step2_5QP_out/$pheno/$cohort/$file_bn
